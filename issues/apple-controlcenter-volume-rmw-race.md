...
| **Trigger (necessary, not causal)** | **Alcove 1.7.9** (build 203, `com.henrikruscon.Alcove`) running; kicked off by a Spotify Connect device transfer |
...
Both the **direction** (up or down) and the **device** (built-in speakers, Bluetooth) vary between occurrences. Every write during the runaway comes from **ControlCenter's own threads** — the third-party app that is a necessary precondition issues **zero** volume writes while the loop runs.

**Hearing-safety relevance:** the ratchet advances one 1/16 step per ~33 ms, so the up-direction case drives output to full scale in **≈0.5 s with no ramp**. The reporter hit this with the transfer target live and in use.

系统音量失控:单向棘轮到 0%(并静音)或 100% 并卡死,ControlCenter 以约 30 Hz 无限写入音量属性。手动调节会被立即覆盖 —— 棘轮只是从新值重新开始。方向(上/下)和设备(内置扬声器/蓝牙)在两次事故间都不同。失控期间的每一条写入都来自 ControlCenter 自己的线程。
...
| Device | `100` — `BuiltInSpeakerDevice` | `C4-B3-49-AA-E7-97` — Bluetooth audio |
| Writes to the *other* device | — | **0 writes to device 100** |
...
10:39:27.285  set system volume: 0.490000 -> 0.500000   ← another thread lands a stale value, ratchet restarts
...
2. **Distinct threads do the writing.** In incident 2, seven ControlCenter TIDs (`54625`, `5498c`, `5467e`, `5386f`, `5498a`, `5499c`, `5498e`) all issue `AudioObjectSetPropertyData`. The bookkeeping line (`set system volume: X -> Y`) comes from a *different* thread (`1b1b`) than the threads doing the CoreAudio writes.
3. **Stale values appear mid-ramp.** `0.435000` and `0.490000` are not multiples of 1/16 — they are intermediate states observed by a thread that read while another was writing.
4. **No HID input.** A 6-second HID capture during an active runaway returned **one** key event total. No `NX_KEYTYPE_SOUND_UP` stream, no injected events.
5. **The loop is closed inside ControlCenter.** Each iteration is `syncMute → updateMute → setLevel → set system volume → present VolumeSystemBannerContent`, all in-process.
...
10:39:26.849  ControlCenter set system volume: 0.435 -> 0.5        ← +203 ms, runaway begins
```
