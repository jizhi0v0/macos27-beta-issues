...
- **Separate bug found alongside, not the cause:** `imagent` (`IMCore`) is in an **unbounded no-backoff retry loop** against `com.apple.AddressBook.ContactsAccountsService`, blocked by its own sandbox — `Connection init failed at lookup with error 159 - Sandbox restriction` → `Migration service failed database preparation` → immediate retry, **multiple times per millisecond** (timestamps `.307/.307/.308/.308`). **44,537 errors in 4 h** — but only **14 s of CPU**. It is log noise, not a CPU contributor. Worth its own Feedback; do not conflate with this issue.
