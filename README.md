...
| *(new)* | [`ecosystemd` trust-anchor re-evaluation retry loop](issues/apple-ecosystemd-trust-retry-loop.md) | Apple `ecosystemd` (Ecosystem.framework) ↔ Security / `trustd` | 🔴 confirmed on beta4 `26A5388g` — `SecTrustCopyAppleTrustAnchors` **~85×/sec**, 15,885 lines/60 s, **26–57% CPU**, with 1,227 `UNIX error exception: 5` (failing + retrying) | none | Feedback candidate `FB____` |
