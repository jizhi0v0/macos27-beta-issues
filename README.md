...
| *(new)* | [`mds` floods the log with ~2,420 CoreDuet activity lines/sec](issues/apple-mds-coreduet-activity-storm.md) | Apple Spotlight `mds` ↔ CoreDuet / `contextstored` | 🔴 confirmed on beta4 `26A5388g` — **145,241 lines/60 s = 54% of all system log volume**; `mds` 39–43% + `contextstored` 24–39% CPU; `mdworker.shared` respawned **341×/min** | none (silencing the subsystem hides the log, not the CPU) | Feedback candidate `FB____` |
