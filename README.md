...
- 🟡 **Ready to file** — **#20 ControlCenter volume runaway (concurrent RMW race)** ([draft](feedback/controlcenter-volume.md)): caught live ×2 on one boot, ratcheting **up to 100% *and* down to 0%+mute**, on **different devices** (built-in speakers / Bluetooth) — direction and device both arbitrary, which is what rules out a stuck key and points at a lost-update race across **≥7 ControlCenter threads**. Ratchet step is exactly 1/16, ~33 ms apart → full scale in **~0.5 s, no ramp** (hearing-exposure hazard; lead with this). Every write during the runaway is ControlCenter's own — the third-party precondition (**Alcove 1.7.9**) issues **0** writes in incident 2. **Not a 27 regression**: same up-direction symptom on **26.3.1** via [Alcove #675](https://github.com/henrikruscon/alcove-releases/issues/675), which was closed stale and whose repo is now archived read-only — no upstream channel left. Draft flags its own two limits (the "quit Alcove" result is uncounted; no first-discovery claim).
