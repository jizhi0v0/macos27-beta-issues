...
| [17](https://github.com/jizhi0v0/macos27-beta-issues/issues/17) | [ViewBridge `NSRemoteView` uncaught ObjC exception on sheet order-on-screen (WeChat image viewer + CleanShot X QuickLook + DingTalk + duo-pasted)](issues/wechat-imageviewer-viewbridge-crash.md) | Apple ViewBridge / AppKit (`NSRemoteView`) — hits WeChat 4.1.11 (Chromium), CleanShot X 4.8.9 (QuickLook), DingTalk 8.3.15 (Qt) **&** duo-pasted (Swift/AppKit) | 🔴 confirmed **cross-app, 4 apps / 4 UI toolkits**, **31 crashes** (WeChat ×26 + CleanShot X ×3 + DingTalk ×1 + duo-pasted ×1, byte-identical signature at `+216`) — **12 of them on beta4 `26A5388g`** (07-21 → 08-04) | none confirmed (retry recovers; routine in practice) | Feedback candidate `FB____` |
